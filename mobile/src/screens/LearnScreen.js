...
const LearnScreen = () => {
  ...
  const categories = [
    ...
    { id: 'behavioral', name: 'Behavioral Finance', icon: 'brain-outline' },
    { id: 'personal_finance', name: 'Personal Finance', icon: 'home-outline' },
    { id: 'advanced', name: 'Advanced Strategies', icon: 'trending-up-outline' },
  ];
  ...
  const handleStartCourse = async (course) => {
    ...
    await AnalyticsService.getInstance().trackLearningStart(course.id, course.category);
    
    // Navigate to course detail with category and module info
  ...
  const getAllModules = () => {
    ...
    learningData.courses.forEach(category => {
      if (category.modules) {
  ...
  const filteredCourses = getAllModules().filter(course => {
